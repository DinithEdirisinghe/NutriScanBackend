// ENHANCED AI + FORMULA SCORING TEST - HEALTHY PERSON
// Tests the hybrid AI + formula approach for someone with no health conditions
...
class EnhancedScoringService {
  ...
  scoreSugarEnhanced(sugars, fiber, context, user) {
    ...
    if (user?.hasDiabetes) {
      ...
      // Healthy person - more lenient with sugar
  ...
  scoreFatEnhanced(totalFat, saturatedFat, transFat, context, user) {
    ...
    if (saturatedFat) {
      if (user?.hasHighCholesterol) {
        ...
        // Healthy person - more lenient with saturated fat
  ...
  scoreSodium(sodium, user) {
    ...
    if (user?.hasHighBloodPressure) {
      ...
      // Healthy person - more lenient with sodium
  ...
  scoreCalories(calories, user) {
    ...
    if (bmiCategory === 'obese') {
      ...
      // Healthy person - very lenient with calories
  ...
  calculateAdjustments(context, user) {
    ...
    // Glycemic penalty only for diabetics
    if (user?.hasDiabetes) {
...
// Test data - same 10 foods but ranked for a HEALTHY person
const testFoods = [
  {
    name: 'Salmon Fillet',
...
function runTest() {
  console.log('\n=================================================================');
  console.log('  ENHANCED AI + FORMULA TEST - HEALTHY PERSON (NO CONDITIONS)');
  console.log('=================================================================\n');
  ...
  console.log(`👤 Test Patient: Healthy with BMI ${testPatient.bmi.toFixed(1)} (${testPatient.bmiCategory})`);
  console.log(`   No diabetes, no high cholesterol, no high blood pressure`);
  console.log(`⚖️  Weights: BALANCED - Sugar ${Math.round(weights.sugar * 100)}%, Fat ${Math.round(weights.fat * 100)}%, Sodium ${Math.round(weights.sodium * 100)}%, Calories ${Math.round(weights.calorie * 100)}%, Quality ${Math.round(weights.quality * 100)}%`);
  console.log('\n=================================================================\n');
  ...
  // Print rankings
  console.log('EXPECTED RANKING (Common Sense for Healthy Person):\n');
  [...results].sort((a, b) => a.expectedRank - b.expectedRank).forEach(food => {
    ...
    console.log(`${emoji} #${food.expectedRank} - ${food.name} (Sugar: ${food.nutrients.sugars}g, Calories: ${food.nutrients.calories})`);
  });
  ...
  sortedResults.forEach((food, index) => {
    ...
    console.log(`   Scores: Sugar ${food.breakdown.sugar}, Fat ${food.breakdown.fat}, Sodium ${food.breakdown.sodium}, Calorie ${food.breakdown.calorie}, Quality ${food.breakdown.quality}`);
  ...
  console.log('=================================================================');
  console.log('\n💡 KEY INSIGHT: Healthy people have BALANCED weights (20% each)');
  console.log('   They can enjoy more variety without harsh penalties!\n');
  console.log('=================================================================\n');
}
